...
export async function POST(request: NextRequest) {
  try {
    ...
    const subjects: Subject[] = subjectsRaw.map(s => ({
      ...
      semester: s.semester || activeSemester,
    ...
    if (result.schedules.length > 0) {
      ...
      const scheduleData = result.schedules.map(s => ({
        ...
        semester: activeSemester, // Use active semester from System Settings
        academicYear: activeAcademicYear, // Use active academic year from System Settings
      }));
    ...
    for (const schedule of result.schedules) {
      if (!notifiedFaculty.has(schedule.facultyId)) {
        ...
        if (facultyMember) {
          ...
          const notificationMessage = `You have been assigned ${facultySchedules.length} class(es) totaling ${totalUnits} units for ${activeSemester} ${activeAcademicYear}. Please review your schedule in the calendar view.`;

...
import { 
  ...
  FileEdit, Settings, Plus, Edit3, Loader2
} from 'lucide-react';
...
export function ConflictsView() {
  ...
  const columns: ColumnDef<ConflictWithDetails>[] = [
    ...
    { accessorKey: 'resolved', header: 'Status', cell: ({ row }) => <Badge variant={row.original.resolved ? 'default' : 'destructive'}>{row.original.resolved ? 'Resolved' : 'Active'}</Badge> },
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Total</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{conflicts.length}</div></CardContent></Card>
        <Card className="border-red-500/50"><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Critical</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-red-500">{criticalConflicts.length}</div></CardContent></Card>
        <Card className="border-amber-500/50"><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Warnings</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-amber-500">{warningConflicts.length}</div></CardContent></Card>
        <Card className="border-emerald-500/50"><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Resolved</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-emerald-500">{resolvedConflicts.length}</div></CardContent></Card>
      </div>

      {criticalConflicts.length > 0 && (
        <Card className="border-red-500/50">
          <CardHeader><CardTitle className="flex items-center gap-2 text-red-600"><AlertTriangle className="h-5 w-5" />Critical Conflicts</CardTitle><CardDescription>Must be resolved immediately</CardDescription></CardHeader>
          <CardContent><DataTable columns={columns} data={criticalConflicts} searchKey="description" searchPlaceholder="Search..." /></CardContent>
        </Card>
      )}

      {warningConflicts.length > 0 && (
        <Card className="border-amber-500/50">
          <CardHeader><CardTitle className="flex items-center gap-2 text-amber-600"><AlertCircle className="h-5 w-5" />Warning Conflicts</CardTitle><CardDescription>May affect scheduling</CardDescription></CardHeader>
          <CardContent><DataTable columns={columns} data={warningConflicts} searchKey="description" searchPlaceholder="Search..." /></CardContent>
        </Card>
      )}

      {infoConflicts.length > 0 && (
        <Card className="border-blue-500/50">
          <CardHeader><CardTitle className="flex items-center gap-2 text-blue-600"><Info className="h-5 w-5" />Informational</CardTitle><CardDescription>Algorithm handles automatically</CardDescription></CardHeader>
          <CardContent><DataTable columns={columns} data={infoConflicts} searchKey="description" searchPlaceholder="Search..." /></CardContent>
        </Card>
      )}

      {activeConflicts.length === 0 && (
        <Card><CardContent className="flex flex-col items-center justify-center py-12"><CheckCircle className="h-12 w-12 text-emerald-500 mb-4" /><p className="font-medium text-lg">No Active Conflicts</p><p className="text-muted-foreground">All schedules are conflict-free</p></CardContent></Card>
      )}
